...
CantusFirmus::CantusFirmus(Home home, int size, vector<int> cf, Stratum* low, int v_type, vector<int> m_costs, vector<int> g_costs, vector<int> s_costs, int nV) :
    ...
    if(nV==TWO_VOICES){
        /// 1.H2 from Thibault: The first harmonic interval must be a perfect consonance
        if (activeConstraints[CF_1H2_2V]) {
            dom(home, h_intervals[0], IntSet(IntArgs(PERFECT_CONSONANCES)));
        }

        /// 1.H3 from Thibault: The last harmonic interval must be a perfect consonance
        if (activeConstraints[CF_1H3_2V]) {
